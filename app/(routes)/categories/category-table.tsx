...
const CategoryData = (props: IProps) => {
    ...
    const onSearchChange = React.useCallback(async (value?: string) => {
        if (value) {
            ...
            filterData.map((itemHasChildren) => setHasChildrenDetect(itemHasChildren.hasChildren))
        ...
        switch (columnKey) {
            ...
            case 'child category':
                return (
                    <Chip size='md'
                          variant='shadow'
                          className='flex flex-col  cursor-pointer capitalize border-none gap-1 text-default-900 text-[30px]'>
                        <Link href={user?.childCategories
                            ?.length  ? "/category-details" : "/categories"}>
                            <p className='text-bold  text-lg capitalize text-default-400 '>
                                { user?.childCategories
                                    ?.length  ? 'data' : 'no data'}
                            </p>
                        </Link>
                    </Chip>
